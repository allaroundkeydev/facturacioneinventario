...
class FacturaController extends Controller
{
    ...
    public function create(Request $request): View
    {
        ...
        return view('dte.factura.create', compact(
            'tipo', 'clientes', 'sucursales', 'cajas', 'productos', 'empresa', 'formasPago'
        ));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        // Validación con descuentos y pagos
        $validated = $request->validate([
            ...
            'pagos.*.codigo' => ['required', 'string', 'exists:formas_pago,codigo'],
            'pagos.*.montoPago' => ['required', 'numeric'],
...
$receptorPayload = [
    ...
    'correo'   => $cliente->correo ?? ($validatedReceptor['correo'] ?? null),
        ...
        // 3) Ítems: Procesamiento de descuentos
        $itemsOriginales = $validated['items'];
        $descuentoGeneral = floatval($validated['descuento_general'] ?? 0);
        $ivaPct = floatval($validated['iva'] ?? 13.0);

        // Calcular el total bruto para distribuir el descuento general
        $totalVentaBruta = 0;
        foreach ($itemsOriginales as $item) {
            $totalVentaBruta += floatval($item['cantidad']) * floatval($item['precio']);
        }

        $itemsParaBuilder = [];
        foreach ($itemsOriginales as $item) {
            $itemDescuentoIndividual = floatval($item['descuento'] ?? 0);
            $itemValorBruto = floatval($item['cantidad']) * floatval($item['precio']);
            ...
            // El descuento total por item es la suma del individual y la parte del general
            $montoDescuTotalItem = $itemDescuentoIndividual + $descuentoProporcional;

            $itemsParaBuilder[] = [
                'descripcion' => $item['descripcion'],
                'cantidad' => $item['cantidad'],
                'precio' => $item['precio'],
                'montoDescu' => $montoDescuTotalItem,
                // Pasar otros campos si los hubiera (ej. codigo, uniMedida)
                'codigo' => $item['codigo'] ?? null,
            ];
        }

        $builder->setItems($itemsParaBuilder, $ivaPct);

        // 4) Pagos
        $pagos = $validated['pagos'];
        $builder->setPagos($pagos);

        // 5) Identificación: tipoDte, numeroControl, codigoGeneracion y ambiente
        ...
        Log::info('Preparando DTE - resumen', [
            ...
            'numeroControl' => $numeroControl,
            'codigoGeneracion' => $codigoGeneracion,
            'items_count' => count($itemsParaBuilder),
